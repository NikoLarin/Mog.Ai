...
- `POST /api/v1/payments/create-checkout`: creates one-time Stripe Checkout session ($4.99 by default).
  - Checkout redirect URLs are always forced to the stable production domain (`https://mogmax.org`) outside local development, preventing post-payment `DEPLOYMENT_NOT_FOUND` redirects from expired preview deployments.
